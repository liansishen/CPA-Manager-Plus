...
func publicCustomQuotaConfig(cfg store.ManagerCustomQuotaConfig) store.ManagerCustomQuotaConfig {
	if cfg.Bindings == nil {
		return cfg
	}
	publicBindings := make(map[string]store.ManagerCustomQuotaBinding, len(cfg.Bindings))
	for key, binding := range cfg.Bindings {
	...
	return store.ManagerCustomQuotaConfig{
...
func PublicConfig(cfg store.ManagerConfig) PublicManagerConfig {
	return PublicManagerConfig{
		...
		CustomQuota:          publicCustomQuotaConfig(cfg.CustomQuota),
